...
async def test_graphql_filter(
    query_name, graphql_query, impersonate_user="robwol@54mv4c.onmicrosoft.com"
):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                graphql_url, json=graphql_query, headers=headers, timeout=30.0
            )
            ...
            if response.status_code == 200:
                ...
                if "data" in result and "accessRequests" in result["data"]:
                    ...
                    print(
                        f"[SUCCESS] Found {total} total requests, {len(access_requests)} returned"
                    )
                    ...
                    for i, req in enumerate(access_requests[:3], 1):
                        ...
                        print(
                            f"     Beneficiary: {beneficiary.get('displayName', 'N/A')} (ID: {beneficiary.get('identityId', 'N/A')})"
                        )
                        print(
                            f"     Resource: {req.get('resource', {}).get('name', 'N/A')}"
                        )
...
async def main():
  ...
  accessRequests(beneficiaryIdentityId: $beneficiaryIdentityId) {
    ...
    data {
      ...
      resource {
        ...
        "variables": {"beneficiaryIdentityId": "YANLER"},
    }
  ...
  accessRequests(filter: "beneficiary/identityId eq 'YANLER'") {
    ...
    await test_graphql_filter(
        "Filter by beneficiaryIdentityId (Args)", filter_by_identity_id_args
    )
    await test_graphql_filter("Filter with Where Clause", filter_where_identity_id)
    await test_graphql_filter(
        "Filter by Beneficiary ID (GUID)", filter_by_beneficiary_id
    )
