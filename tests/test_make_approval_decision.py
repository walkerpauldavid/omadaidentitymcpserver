...
async def test_step_1():
    ...
    try:
        ...
        if data.get("status") == "success":
            print(
                f"✅ Success! Found {data.get('total_approvals', 0)} pending approvals"
            )
...
async def test_step_3(
    survey_id=None, survey_object_key=None, workflow_step=None, approval_reason=None
):
    ...
    if not survey_id or not survey_object_key:
        ...
        survey_id, survey_object_key, workflow_step, approval_reason = (
            await test_step_2()
        )
        print()
    ...
    if not survey_id or not survey_object_key:
        print(
            "❌ Cannot proceed without survey IDs. Run Step 2 first or provide valid IDs."
        )
        return
    ...
    try:
        ...
        if data.get("status") == "success":
            ...
            print(
                f"   Questions Successfully Submitted: {data.get('questions_successfully_submitted', False)}"
            )
            print()
            ...
            print()
            print(
                f"💡 TIP: Check the log file for the GraphQL mutation and error details"
            )
        else:
...
async def test_step_4():
    ...
    try:
        ...
        if (
            ...
            and data.get("error_type") == "ValidationError"
        ):
...
async def main():
    ...
    print(
        "  python test_make_approval_decision.py        # Run tests 1,2 (default, safe)"
    )
