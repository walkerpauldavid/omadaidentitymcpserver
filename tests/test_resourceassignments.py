...
async def test_resourceassignments_count():
    ...
    result = await query_omada_entity(
        entity_type="RESOURCEASSIGNMENT", count_only=True, include_count=True
    )
    print_resourceassignments_count_result(
        result, "Test 1: Count All Resource Assignments", "Resource Assignment"
    )
...
async def test_resourceassignments_sample():
    ...
    result = await query_omada_entity(
        entity_type="RESOURCEASSIGNMENT", top=5, include_count=True
    )
    display_resource_assignment_result(
        result, "Test 2: Sample Resource Assignments (Top 5)"
    )
