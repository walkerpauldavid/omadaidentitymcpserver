    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                return (
                    f"[FAILED] {entity_type}: Bad request (400) - {response.text[:100]}"
                )
        ...
        if resource_type_id:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    count_value = response.text.strip()
                    print(
                        f"[SUCCESS] Filtered Resource Count (Application Roles): {count_value} objects"
                    )
                else:
                    print(
                        f"[FAILED] Filtered Resource Count: HTTP {response.status_code}"
                    )
        else:
            print(
                "[WARNING]  Filtered Resource Count: No RESOURCE_TYPE_APPLICATION_ROLES configured"
            )
