...
async def test_new_field_filters():
    ...
    display_test_result(
        result, "Test 1: New Generic Field Filter (FIRSTNAME eq 'Emma')"
    )
    ...
    display_test_result(
        result,
        "Test 2: Multiple Field Filters (FIRSTNAME eq 'Emma' AND LASTNAME startswith 'T')",
    )

    # Test 3: Different operators
    result = await query_omada_entity(
        entity_type="Identity",
        field_filters=[{"field": "FIRSTNAME", "value": "Emma", "operator": "ne"}],
        top=5,
    )
    ...
    result = await query_omada_entity(
        ...
        field_filters=[
            {
                "field": "DESCRIPTION",
                "value": "The Omada Identity System",
                "operator": "eq",
            }
        ],
        top=2,
...
async def test_backward_compatibility():
    ...
    result = await query_omada_entity(
        entity_type="Identity", firstname="Emma", firstname_operator="eq", top=3
    )
...
async def test_comparison():
    ...
    try:
        ...
        print(
            f"Results match: {data_old.get('entities_returned') == data_new.get('entities_returned')}"
        )
