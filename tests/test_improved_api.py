...
async def demo_old_vs_new():
    ...
    print(
        "query_omada_entity(entity_type='System', firstname='test')  # Makes no sense!"
    )
    ...
    print("    field_filters=[")
    print(
        "        {'field': 'DESCRIPTION', 'value': 'Identity System', 'operator': 'contains'},"
    )
...
async def demo_real_queries():
    ...
    if data.get("status") == "success":
        print(
            f"[SUCCESS] Found {data.get('entities_returned', 0)} identities (not Emma)"
        )
        print(f"   Filter: {data.get('filter', 'N/A')}")
    else:
        print(f"[ERROR] {data.get('status', 'Unknown')}")
