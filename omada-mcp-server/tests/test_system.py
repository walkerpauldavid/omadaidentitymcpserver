    ...
    try:
        ...
        if "data" in data and "value" in data["data"]:
            ...
            for i, system in enumerate(data["data"]["value"], 1):
                ...
                system_type = (
                    system.get("SYSTEMTYPE", {}).get("Value", "N/A")
                    if isinstance(system.get("SYSTEMTYPE"), dict)
                    else system.get("SYSTEMTYPE", "N/A")
                )
...
def print_system_count_result(result_json, test_name, object_type):
    """Print system count result with object count and type information."""
    try:
        data = json.loads(result_json)
        if data.get("status") == "success":
            ...
            display_system_result(result_json, test_name)
...
async def test_system_count():
    ...
    result = await query_omada_entity(
        entity_type="System", count_only=True, include_count=True
    )
    print_system_count_result(result, "Test 1: Count All Systems", "System")


async def test_system_sample():
    """Test getting a sample of systems"""
    result = await query_omada_entity(entity_type="System", top=10, include_count=True)
        ...
        if (
            ...
            for key, value in system.items():
                if isinstance(value, dict) and "Value" in value:
                    ...
                    print(
                        f"{key}: {type(value).__name__} with keys: {list(value.keys())}"
                    )
...
async def test_system_field_discovery():
    ...
    for field in test_fields:
        try:
            result = await query_omada_entity(
                entity_type="System", filter_condition=f"{field} ne null", top=1
            )
