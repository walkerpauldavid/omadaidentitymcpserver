    ...
    try:
        ...
        if (
            ...
            for i, assignment in enumerate(
                ...
                identity_name = (
                    identity_info.get("DISPLAYNAME", "N/A") if identity_info else "N/A"
                )
                ...
                resource_name = (
                    resource_info.get("DISPLAYNAME", "N/A") if resource_info else "N/A"
                )
                ...
                resource_type_name = (
                    resource_type_info.get("DISPLAYNAME", "N/A")
        ...
        try:
            ...
            print(
                clean_result[:500] + "..." if len(clean_result) > 500 else clean_result
            )
        except:
...
async def test_count_all_calculated_assignments():
    ...
    result = await query_omada_entity(
        entity_type="CalculatedAssignments", count_only=True, include_count=True
    )
    print_assignments_count_result(
        result, "Test 1: Count All Calculated Assignments", "Calculated Assignment"
    )
...
async def test_calculated_assignments_wrapper():
    ...
    result = await query_calculated_assignments(
        identity_id=1006500, top=5, include_count=True
    )
    display_assignments_result(
        result, "Test 2: Calculated Assignments via Wrapper Function"
    )
...
async def test_calculated_assignments_custom_select():
    ...
    display_assignments_result(
        result, "Test 4: Custom Select Fields (AssignmentKey,AccountName,Status)"
    )
...
async def test_calculated_assignments_no_expand():
    ...
    result = await query_calculated_assignments(
        identity_id=1006500, expand="", top=3  # No expand
    )
